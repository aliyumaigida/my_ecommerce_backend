...
from .serializers import CartItemSerializer, CartSerializer, CategoryDetailSerializer, CategorylistSerializer, ProductDetailSerializer, ProductlistSerializer, ReviewSerializer, WishlistSerializer
...
def add_review(request):
    ...
    if Review.objects.filter(product=product, user=user).exists():
        return Response("You ready drop a review for this product.", status=400)
...
def update_review(request, pk):
    ...
    rating = request.data.get("rating")
    review_tex = request.data.get("review")

    review.rating = rating
    review.review =review_tex
    review.save()
...
def create_checkout_session(request):
    ...
    try:
        cart_obj = Cart.objects.get(cart_code=cart_code)
        ...
        line_items = [
            ...
            for item in cart_obj.cartitems.all()
        ]
